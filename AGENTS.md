...
Current repo state: this repository is mostly product and stack documentation plus an HTML mockup. It does not yet contain the Apps Script source scaffold, Node tooling, clasp config, or sheet schema files.
...
- `personal_finance_logging_web_app_specs.md`: main product spec
- `google_sheets_2_tier_expense_categories.md`: expense category taxonomy
- `google-sheets-apps-script-git-clasp-stack.md`: recommended Apps Script + clasp + Git workflow
- `mockup.html`: current visual/prototype reference
...
Use the sheet structure from `personal_finance_logging_web_app_specs.md` unless the user explicitly changes it.

## Category Rules

- Expense categories use two tiers. Use `google_sheets_2_tier_expense_categories.md` as the baseline taxonomy.
...
Follow the clasp stack doc when initializing code:
...
Until tooling exists, do not claim that `npm run check`, `clasp`, or CI is available.

After the Node/clasp scaffold exists, expected checks are:
...
- Should `mockup.html` remain only a prototype reference, or should its UI be migrated into the Apps Script HTML service as the starting point?
